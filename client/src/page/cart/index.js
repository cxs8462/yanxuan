...
import SelectAdress from "../../components/addressSet/selectAdress";
class Cart extends Component{
    ...
    setAddressid(id){
        const {cartList} = this.state
        const data = cartList.map(r=>({...r,addressid : id}))
        this.setState(state=>({cartList: data}),this.postData)
    }
    postData(){
        const {cartList} = this.state
        const {actions:{startLoading, endLoading}} = this.props
        startLoading()
        http.postPushOrder({list:cartList}).then(r=>{
            endLoading()
            this.fetchCartData()
        })
    }
    ...
    render ()   {
        ...
        return (
          <div id="cart">
              <SelectAdress refs={this.ref} that={this}/>
